...
class GraphicalGame(Game):
    def __init__(self, hai_set, players, start_point, view=None, open=False):
        super().__init__(hai_set, players, start_point)

        if open:
            self.screen = mg.Screen(self.game_reader, players, view, "Mahjong")
        else:
            self.screen = mg.Screen(self.game_reader, [view], view, "Mahjong")
    ...
    # ツモ
    def tsumo_pop(self, player):
        super().tsumo_pop(player)
        self.screen.draw()

    # 打牌
    def dahai(self, player, index):
        hai = super().dahai(player, index)
        self.screen.draw()
        return hai
    ...
    # ロン
    def ron(self, player, target, whose):
        super().ron(player, target, whose)
        self.screen.draw()

    # 暗槓
    def ankan(self, player, hais):
        super().ankan(player, hais)
        self.screen.draw()

    # 加槓
    def kakan(self, player, hai):
        super().kakan(player, hai)
        self.screen.draw()

    # 明槓
    def minkan(self, player, hais, target, whose):
        super().minkan(player, hais, target, whose)
        self.screen.draw()

    # ポン
    def pon(self, player, hais, target, whose):
        super().pon(player, hais, target, whose)
        self.screen.draw()

    # チー
    def chi(self, player, hais, target, whose):
        super().chi(player, hais, target, whose)
        self.screen.draw()
